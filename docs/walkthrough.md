# Walkthrough: Building a Waitlist App with UpToCode

This is a complete worked example. You'll build a simple waitlist app — people sign up, an admin invites them — and see UpToCode catch real bugs as Claude writes the code.

**What you'll see:**
- UpToCode interviewing you about what you're building
- UpToCode turning your answers into an enforceable spec
- Claude writing code that breaks a rule
- UpToCode catching it in the same turn, before it becomes a bug
- The fix
...
UpToCode will catch all three.

---

## Step 1: Install UpToCode and open your project

Follow the [installation steps](../README.md#installation), then open Claude Code in a new empty project folder.

---

## Step 2: Let UpToCode interview you

Say to Claude:

> *"Interview me to build my spec"*

UpToCode starts asking you questions one at a time. Here's what that conversation looks like:

---

**UpToCode:** What does your app do? Describe it in one or two sentences, like you'd explain it to a friend.

**You:** It's a waitlist. People sign up with their email, and I can invite them when I'm ready to let them in.

---

**UpToCode:** Who uses the app? Is there just one type of user, or are there different roles — like regular users and admins?

**You:** Two types. Regular people who sign up from the landing page, and me as the admin who decides who gets invited.

---

**UpToCode:** What's the main thing your app tracks or stores?

**You:** Signups. Each one has an email address and a name.

---

**UpToCode:** For each signup — what information do you need to store about it?

**You:** Email, name, when they signed up, and whether they've been invited yet or rejected.

---

**UpToCode:** Does anything go through stages or statuses?

**You:** Yes — a signup starts as pending, then either gets invited or rejected.

---

**UpToCode:** What are the key things someone can do in the app?

**You:** Public: submit your email to join the waitlist. Admin: view all signups, send an invitation, or reject someone.

---

**UpToCode:** Are there any rules the app must enforce?
...
**UpToCode:** Does the app connect to any outside services?

**You:** SendGrid for sending the invitation emails.

---

**UpToCode:** What secret keys or environment variables does the app need?

**You:** SENDGRID_API_KEY.

---

**UpToCode:** Great, I have everything I need. Let me build your spec.

---

UpToCode takes your answers and generates a `requirements.md` in your project folder. You never have to write or read it — it's there so UpToCode can enforce the rules while Claude codes.
...
Looks reasonable. But UpToCode sees two problems.

---

## Step 4: UpToCode catches the bugs

The moment Claude writes that file, UpToCode fires automatically. Claude sees this feedback in the same turn:

```
UpToCode: 2 rule violation(s) in app.py
...
UpToCode runs again. No violations. Claude moves on.
...
UpToCode reads your spec and produces `adversarial-tests.md` — a list of inputs designed to break the app:
...
UpToCode checks whether any route is touching data it shouldn't. In this example, if the public signup endpoint accidentally had access to send invites, UpToCode would catch it:
...
You answered nine plain-English questions. UpToCode turned your answers into a set of rules. Every time Claude wrote something that broke one of those rules, UpToCode flagged it immediately — in the same turn, before it could become a bug.
...
UpToCode will read your code and write the spec for you, then you're enforced from that point forward.
